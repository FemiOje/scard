...
// import * as models from "./models.gen"; // Unused import

export function setupWorld(provider: DojoProvider) {
  const build_game_systems_createGame_calldata = (
    gameId: BigNumberish
  ): DojoCall => {
    return {
  ...
  const game_systems_createGame = async (
  ...
  const build_game_systems_fight_calldata = (
    gameId: BigNumberish
  ): DojoCall => {
    return {
      contractName: "game_systems",
  ...
  const game_systems_fight = async (
    snAccount: Account | AccountInterface,
    gameId: BigNumberish
  ) => {
    try {
  ...
  const build_game_systems_getPosition_calldata = (
    gameId: BigNumberish
  ): DojoCall => {
    return {
      contractName: "game_systems",
  ...
  const game_systems_getPosition = async (gameId: BigNumberish) => {
    try {
      return await provider.call(
        "scard",
        build_game_systems_getPosition_calldata(gameId)
      );
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const build_game_systems_move_calldata = (
    gameId: BigNumberish,
