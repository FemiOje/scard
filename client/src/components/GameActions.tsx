...
export function GameActions() {
  ...
  // const [moves, setMoves] = useState<Moves | null>(null); // Moves model doesn't exist
  ...
  const fetchPlayerState = async () => {
    ...
    try {
      ...
      const positionData = await client.getEntity("scard", "Position", [
        address,
      ]);
      if (positionData) {
  ...
  if (status !== "connected" || !address) {
    return (
      ...
      >
        <h2>Game Actions</h2>
        <p style={{ color: "#666" }}>
          Please connect your wallet to interact with the game.
        </p>
      </div>
    );
  }
        ...
        >
          ...
          {position && (
            <div style={{ marginBottom: "0.5rem" }}>
              <strong>Position:</strong> ({Number(position.x)},{" "}
              {Number(position.y)})
            </div>
          )}
        </div>
      ...
      {lastAction && (
        <div
          style={{
            marginTop: "1rem",
            padding: "1rem",
            backgroundColor: lastAction.startsWith("Error")
              ? "#f8d7da"
              : "#d4edda",
