...
mod tests {
    ...
    use scard::models::{
        Beast, BeastEncounter, BeastEncounterTrait, CurrentEncounter, CurrentEncounterTrait,
        Direction, Encounter, GameStateTrait, Player, PlayerTrait, Position,
    };
    use scard::systems::game::contracts::{IGameSystemsDispatcher, IGameSystemsDispatcherTrait};
    ...
    fn namespace_def() -> NamespaceDef {
        let ndef = NamespaceDef {
            ...
            resources: [
                ...
                TestResource::Event("FledEvent"), TestResource::Contract("game_systems"),
            ]
                .span(),
        };
        ndef
    ...
    // #[test]
    // #[available_gas(l1_gas: 0, l1_data_gas: 40000, l2_gas: 70000000)]
    ...
    //     assert(found_attack_points, 'not found AttackPoints');
    // }
    ...
    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 30000, l2_gas: 600000000)]
    ...
    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 40000, l2_gas: 700000000)]
        ...
        while i < 30 {
            ...
            match encounter_enum {
                Encounter::Werewolf => {
                    ...
                    assert(
                        beast.attack_points >= 20 && beast.attack_points <= 30, 'werewolf attack',
                    );
                    assert(
                        beast.damage_points >= 10 && beast.damage_points <= 20, 'werewolf damage',
                    );
                    found_beast = true;
                    break;
                },
                Encounter::Vampire => {
                    ...
                    assert(
                        beast.attack_points >= 25 && beast.attack_points <= 35, 'vampire attack',
                    );
                    assert(
                        beast.damage_points >= 15 && beast.damage_points <= 25, 'vampire damage',
                    );
                    found_beast = true;
                    break;
                },
    ...
    // #[test]
    // #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 50000000)]
    // fn test_flee_successful() {
    //     // Test successful flee where player survives damage and escapes
    //     let game_id: u64 = 400;
    //     let ndef = namespace_def();
    //     let mut world = spawn_test_world([ndef].span());
    //     world.sync_perms_and_inits(contract_defs());
    ...
    //     // Ensure player will survive (has enough health)
    //     // If beast damage >= health, damage player to ensure survival
    //     if beast_damage >= health_before {
    //         let mut player: Player = world.read_model(game_id);
    //         PlayerTrait::apply_damage(ref player, beast_damage - 1); // Leave player with 1
    //         health world.write_model(@player);
    //     }

    //     // Get updated player health
    //     let player_before_flee: Player = world.read_model(game_id);
    //     let health_before_flee = player_before_flee.health;

    //     // Flee from beast
    //     game_systems.flee(game_id);

    //     // Verify player took damage but survived
    //     let player_after: Player = world.read_model(game_id);
    //     let expected_health: u32 = if beast_damage >= health_before_flee {
    //         0
    //     } else {
    //         health_before_flee - beast_damage
    //     };
    //     assert(player_after.health == expected_health, 'player health not updated');
    //     assert(player_after.health > 0, 'player should survive');

    //     // Verify encounter is now FreeRoam (flee successful)
    //     let encounter: CurrentEncounter = world.read_model(game_id);
    //     assert(encounter.encounter_type == 8, 'should be FreeRoam'); // 8 = FreeRoam

    //     // Verify game is still in progress (player survived)
    //     let game_state: scard::models::GameState = world.read_model(game_id);
    //     assert(game_state.is_in_progress(), 'game should be in progress');
    //     assert(!game_state.is_lost(), 'game should not be lost');
    // }

    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 50000000)]
    fn test_flee_player_dies() {
        // Test failed flee where player dies from beast damage
        let game_id: u64 = 401;
        let ndef = namespace_def();
        let mut world = spawn_test_world([ndef].span());
        world.sync_perms_and_inits(contract_defs());
        ...
        // Ensure player will die (beast damage >= player health)
        // If beast damage < 10, damage player more
        let player_check: Player = world.read_model(game_id);
        if beast_damage < player_check.health {
            let mut player: Player = world.read_model(game_id);
            // Damage player so beast damage will kill them
            PlayerTrait::apply_damage(ref player, player_check.health - beast_damage + 1);
            world.write_model(@player);
        }
        ...
        // Verify encounter remains as beast (player died during flee)
        let encounter: CurrentEncounter = world.read_model(game_id);
        let encounter_enum: Encounter = encounter.encounter_type.into();
        assert(
            encounter_enum == Encounter::Werewolf || encounter_enum == Encounter::Vampire,
            'encounter should remain beast',
        );
    }

    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 500000000)]
    fn test_flee_free_flee() {
        // Test free flee ability (no damage, guaranteed success)
        let game_id: u64 = 402;
        let ndef = namespace_def();
        let mut world = spawn_test_world([ndef].span());
        world.sync_perms_and_inits(contract_defs());
        ...
        // Grant free flee ability (move until we get FreeFlee encounter)
        let mut found_free_flee = false;
        let mut i: u8 = 0;
        while i < 30 {
            game_systems.move(game_id, Direction::Right);
            let encounter: CurrentEncounter = world.read_model(game_id);
            if encounter.encounter_type == 7 {
                // Found FreeFlee encounter (type 7)
                found_free_flee = true;
                break;
            }
            i += 1;
        ...
        // If we didn't find FreeFlee, grant it manually for testing
        if !found_free_flee {
            let mut player: Player = world.read_model(game_id);
            player.has_free_flee = true;
            world.write_model(@player);
        }

        // Verify player has free flee
        let player_before: Player = world.read_model(game_id);
        assert(player_before.has_free_flee, 'player should have free flee');
        ...
        // Get beast encounter (for context, but damage won't be applied due to free flee)
        let _beast_encounter: BeastEncounter = world.read_model(game_id);

        // Flee from beast (with free flee)
        game_systems.flee(game_id);

        // Verify player took NO damage (free flee)
        let player_after: Player = world.read_model(game_id);
        assert(player_after.health == health_before, 'health unchanged');

        // Verify free flee was consumed
        assert(!player_after.has_free_flee, 'free flee should be consumed');
    ...
    // #[test]
    // #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 50000000)]
    // fn test_flee_free_flee_consumed() {
    //     // Test that free flee ability is consumed after use
    //     let game_id: u64 = 403;
    //     let ndef = namespace_def();
    //     let mut world = spawn_test_world([ndef].span());
    //     world.sync_perms_and_inits(contract_defs());
    ...
    //     // Grant free flee ability manually
    //     let mut player: Player = world.read_model(game_id);
    //     player.has_free_flee = true;
    //     world.write_model(@player);
    ...
    //     // Verify player has free flee
    //     let player_check: Player = world.read_model(game_id);
    //     assert(player_check.has_free_flee, 'player should have free flee');
    ...
    //     // Flee from beast (should consume free flee)
    //     game_systems.flee(game_id);
    ...
    //     // Verify flee was successful (no damage taken)
    //     assert(player_after.health == DEFAULT_PLAYER_HEALTH, 'player health unchanged');
    // }

    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 10000, l2_gas: 20000000)]
    fn test_flee_not_in_progress() {
        // Test that flee fails if game is not in progress
        let game_id: u64 = 404;
        let ndef = namespace_def();
        let mut world = spawn_test_world([ndef].span());
        world.sync_perms_and_inits(contract_defs());
        ...
        // Verify game state is not in progress
        let game_state_check: scard::models::GameState = world.read_model(game_id);
        assert(!game_state_check.is_in_progress(), 'game should not be in progress');
        assert(game_state_check.is_lost(), 'game should be lost');
        // Try to flee - should fail (will panic)
    // We verify the game state is not in progress
    }

    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 10000, l2_gas: 20000000)]
    fn test_flee_not_beast_encounter() {
        // Test that flee fails if not in beast encounter
        let game_id: u64 = 405;
        let ndef = namespace_def();
        let mut world = spawn_test_world([ndef].span());
        world.sync_perms_and_inits(contract_defs());
        ...
        // Verify encounter is not a beast encounter
        let encounter: CurrentEncounter = world.read_model(game_id);
        assert(!encounter.is_beast_encounter(), 'should not be beast encounter');
        // Try to flee - should fail (will panic)
    // We verify the encounter is not a beast encounter
    }

    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 50000000)]
    fn test_flee_damage_underflow_protection() {
        // Test that damage application safely handles underflow
        // Player with very low health vs beast with high damage
        let game_id: u64 = 406;
        let ndef = namespace_def();
        ...
        // Get beast encounter (damage will be applied, handling underflow safely)
        let _beast_encounter: BeastEncounter = world.read_model(game_id);

        // Flee from beast (beast damage will be >= 1, should handle underflow safely)
        game_systems.flee(game_id);
    ...
    #[test]
    #[available_gas(l1_gas: 0, l1_data_gas: 20000, l2_gas: 50000000)]
    fn test_flee_werewolf_vs_vampire() {
        // Test fleeing from different beast types (Werewolf and Vampire)
        let game_id: u64 = 407;
        let ndef = namespace_def();
        let mut world = spawn_test_world([ndef].span());
        world.sync_perms_and_inits(contract_defs());
        ...
        if beast_damage >= health_before {
            let mut player: Player = world.read_model(game_id);
            PlayerTrait::apply_damage(ref player, beast_damage - 1); // Leave player with 1 health
            world.write_model(@player);
        }
